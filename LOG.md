...
- [ ] 扩大重采样窗口（512 或全序列切分）并复跑难题集，验证能否出现"错→对"的救回正例。
- [ ] 在难题集上粗调 `alpha`（2 vs 4）、`gain_threshold`、`patience`、`rejection_patience`。
...
目前完成的是 **5 题 GPU 冒烟实验 + 根因修复 + α=4/v2/v3 三轮复跑，不是 100 题或 500 题完整实验**。工程链路已全部打通：Instruct 模型 + 官方 CoT prompt 下判分可信、每题恰一个 `\boxed{}`、5/5 全对、q5 伪造输出消失；重采样从不破坏正确答案，自适应稳定省算力（v3 下省 14.6% token、13.1% 时间）。当前唯一障碍变成**实验设计层面**：前 5 题对 Instruct 太简单（初始 100%），无法展示准确率增益。下一步直接推进更大规模：在更难/更多的题目上跑 paired 实验（先 5–20 题调参，再 100 题出统计结论）。
