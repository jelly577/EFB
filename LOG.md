...
- [ ] **换用 `Qwen/Qwen2.5-Math-7B-Instruct`（或 base + few-shot 官方协议）解决"停不下来"，再重跑 5 题建立基线**。
...
目前完成的是 **5 题 GPU 冒烟实验 + 根因修复 + α=4 复跑 + 新 prompt v2 复跑，不是 100 题或 500 题完整实验**。采样方法本身已站稳：重采样从不破坏初始正确答案，自适应停止持续省算力（v2 下省 15.5% token、18.3% 时间）。当前阻塞项是**模型-协议不匹配**：base 模型配 chat prompt 会答完不停、自编新题，导致"取最后一个 `\boxed{}`"判分严重低估（真实 80% vs 判分 40%）。下一步：换 Instruct 模型（或 base + few-shot）重建基线，然后调参并推进 100 题 paired 实验。
