...
- 乱码退化从 2/20 降到 1/20（q17 ds=43 仍顶满 2048 无 `\boxed{}`；v4 中另一题本轮正常）。退化与窗口无关，仍待重复惩罚或复跑判定处理。
...
- [ ] 提高 `rejection_patience`（6–8）复跑 adaptive（512 窗口），验证能否抓住 q14 类救回同时保留省算力。
- [ ] 在难题集上粗调 `alpha`（2 vs 4）、`gain_threshold`、`patience`。
- [ ] 处理 2/20 的乱码退化生成（重复惩罚或复跑判定），避免死重题目污染统计。
...
工程链路全部打通且判分可信（Instruct + 官方 CoT prompt），安全底线稳固（v3/v4/v5 均零"对→错"）。**方法的两个核心主张现在各有一个数据支点**：① 自适应省算力——v4 省 21.3% token / 21.2% 时间（128 窗口）、v5 省 19.5%（512 窗口）；② 重采样提准确率——v5 的 512 窗口下出现首个"错→对"（fixed 55% > 初始 50%）。当前主要矛盾：adaptive 的 `rejection_patience=4` 恰在救回前一步停止，暴露"省算力 vs 抓救回"的调参张力。下一步：调 `rejection_patience` 复跑 → 难题集 α 对照 → 100 题 paired 出统计结论（bootstrap / McNemar）。
