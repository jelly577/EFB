...
## 2026-07-20：100 题三臂 paired 实验（v7），首个统计结论

- MATH-500 level 4–5 前 100 题，三臂 paired：fixed 8 步 / adaptive rp=4 / adaptive rp=8，512 窗口 + 复跑判定（`--degeneration-retries 3`），Instruct、α=4、seed=42+位置。服务器迁移到新 AutoDL 实例（westc 区、4080 SUPER）后跑，全程 11:51–18:48（约 7 小时）。输出 `results/fixed_100_w512.jsonl`、`results/adaptive_100_w512_rp4.jsonl`、`results/adaptive_100_w512_rp8.jsonl`、`results/hundred_summary.json`；人工复核见 `results/hundred_review.md`。

**准确率**：初始 67% → fixed 69%、rp4 67%、rp8 69%。

**首次出现"对→错"（三臂同一题：index 28/ds 67），但复核后确认是判分假阴性**：模型把 `\boxed{10}` 改写成数学等价的 `\boxed{10\%}`，`normalize_answer` 未剥离 `\%` 导致误判为错误。三臂命中同一题，说明这不是重采样引入的数学错误——真实安全底线 v3–v7 五轮合计仍是 **0/100+ broken**。已记录为已知判分局限，未静默修改判分脚本。

**"错→对"3 例，均为真实数值跳变（非格式伪影）**：fixed 与 rp8 各命中全部 3 个，rp4 只命中 1 个（漏掉 q2、q25）——**在 20 题（v5/v6）上观察到的"rp4 系统性错过救回"模式，在 100 题上首次获得统计确认**：rp8 与 fixed 逐题正确性 100 题内 0 处分歧（McNemar b10=b01=0），rp4 与 fixed 有 2 处分歧（McNemar p=0.5，样本量不足以显著，但方向与 20 题实验完全一致）。

**bootstrap 置信区间（95%，20000 次重采样）**：fixed/rp8 相对初始生成的准确率提升 = +2.0pp，95% CI [-2.0pp, +6.0pp]——**方向为正但区间跨零，100 题仍不足以在统计上确证增益**，需要更大样本或更强效应量。rp4 相对初始 = 0.0pp，CI [-3.0pp, +3.0pp]。

**省算力**（相对 fixed）：rp4 省 15.9% token / 13.7% 时间（早停率 52%）；rp8 省 6.4% token / 5.8% 时间（早停率 24%）——与 v5/v6 的权衡曲线端点（19.5% vs 4.6%，20 题小样本）方向一致但幅度更平滑，说明此前的极端数字部分是 20 题噪声；100 题给出的 rp4/rp8 曲线更可信。

**退化复跑判定效验**：100 题中 11 题触发复跑，10 题复跑后转正常（含 v5 中曾死重的 ds=41、ds=43），仅 1 题（ds=168）三次复跑后仍退化。死重率从 v5 的 10%（2/20）降到 v7 的 1%（1/100），复跑判定确认有效且未观察到副作用。

**结论**：方法的方向性证据在 100 题上继续成立（rp8 = fixed 的准确率、省 6.4% 算力；rp4 更省但会丢救回），但增益的统计显著性尚未达成——95% CI 跨零。下一步不是急着上 500 题，而是要么扩大到 200–300 题收窄区间，要么先想清楚能不能通过增大效应量（如更宽的窗口、更多重采样步数）而不是单纯堆样本来达到显著性。
...
- [x] 运行 100 题三臂 paired 实验（v7）：rp8 与 fixed 逐题正确性 0 分歧、省 6.4% 算力；rp4 更省（15.9%）但漏 2/3 救回；bootstrap 95% CI 跨零，方向对但未达统计显著。
- [ ] 在难题集上粗调 `alpha`（2 vs 4）、`gain_threshold`、`patience`。
- [ ] 决定下一步是扩大样本（200–300 题）收窄置信区间，还是先加大效应量（更宽窗口/更多步数）再验证。
- [ ] 视需要为 `normalize_answer` 补充 `\%`/`%` 尾部剥离（已发现的判分假阴性，v7 中 1 例误判"对→错"）——目前只记录未修改，需与用户确认是否要动判分脚本。
...
工程链路全部打通且判分可信（Instruct + 官方 CoT prompt），安全底线稳固（v3–v7 五轮，真实数学错误引入 0 次；v7 唯一一次"对→错"经复核确认是判分假阴性）。**100 题 paired 实验（v7）给出了方法目前最强的证据**：adaptive rp=8 与 fixed 逐题正确性完全一致（69% vs 初始 67%），同时省 6.4% 算力——重采样带来的准确率增益不需要用早停去换；rp=4 更省算力（15.9%）但系统性漏掉救回（3 例中只中 1 例），复现了 20 题实验里发现的模式。**唯一的短板是统计显著性**：bootstrap 95% CI 为 [-2.0pp, +6.0pp]，跨零，100 题还不够把方向性证据变成显著结论。下一步二选一：扩大样本到 200–300 题收窄区间，或先加大效应量（更宽窗口/更多步数）再验证——待与用户讨论决定。
