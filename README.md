...
| 退化复跑判定 | ✅ 完成 | 探针证实退化是种子依赖；初始生成换种子复跑（默认 3 次），不动 proposal 分布 |
| 100 题三臂 paired 实验（v7） | ✅ 完成 | rp8 与 fixed 逐题正确性 0 分歧、省 6.4% 算力；bootstrap 95% CI 跨零，方向对但未达统计显著 |
| 固定版/自适应版配对复现 | ✅ 完成 | 同题共享初始生成，停止前共享提议序列 |
| 离线单元测试 | ✅ 33/33 通过 | 2026-07-20 本地重新验证（含选题 7 项、退化复跑 8 项） |
| 1 题 GPU 校正试跑 | ✅ 完成 | 用于确认等长后缀重采样修复 |
| 5 题 GPU 冒烟实验 | ✅ 完成 | RTX 4090 24GB，fixed 与 adaptive 各 5 题 |
| 100 题实验 | ✅ 完成 | 见下方专节；结论方向正确但统计显著性未达成 |
...
- 乱码退化仍是 2/20（q16、q17，每题烧满 6144 token、占全程耗时 16–21%），与窗口大小无关。后续探针证实退化是种子依赖的随机事件，已实现复跑判定（见下）。
...
## 100 题三臂 paired 实验（v7，2026-07-20）

MATH-500 level 4–5 前 100 题，三臂：fixed 8 步 / adaptive rp=4 / adaptive rp=8，512 窗口 + 复跑判定，其余配置与 v5/v6 一致。全程约 7 小时。

| 方法 | 准确率 | 相对 fixed 省 token | 相对 fixed 省时间 | 早停率 |
| --- | ---: | ---: | ---: | ---: |
| 普通初始生成 | 67% | — | — | — |
| 固定 8 次 | 69% | — | — | 0% |
| 自适应 rp=4 | 67% | 15.9% | 13.7% | 52% |
| 自适应 rp=8 | **69%** | 6.4% | 5.8% | 24% |

**核心结论：rp=8 与 fixed 逐题正确性 100% 一致（0 处分歧），同时省 6.4% 算力**——重采样带来的准确率增益不需要靠早停去换。rp=4 更省算力但系统性漏救回（3 例真实"错→对"中只命中 1 例），复现了 20 题实验（v5/v6）里发现的模式，这次在 100 题上首次有统计支持（McNemar 分歧计数）。

**统计显著性尚未达成**：bootstrap 95% 置信区间（20000 次重采样）为 fixed/rp8 相对初始生成 **+2.0pp，CI [-2.0pp, +6.0pp]**——方向正确但区间跨零，100 题还不够把趋势变成显著结论。

**首次出现"对→错"，但确认是判分假阴性**：三臂在同一题（index 28）上都把 `\boxed{10}` 改写成数学等价的 `\boxed{10\%}`，现有判分脚本未剥离尾部 `\%` 而误判为错误——不是重采样引入的真实数学错误。真实安全底线（v3–v7 五轮）仍是 0 broken。详见 [`results/hundred_review.md`](results/hundred_review.md)。
...
results/fixed_100_w512.jsonl         100 题固定版（v7）
results/adaptive_100_w512_rp4.jsonl  100 题自适应 rp=4（v7）
results/adaptive_100_w512_rp8.jsonl  100 题自适应 rp=8（v7）
results/hundred_summary.json         v7 三臂汇总指标
results/hundred_review.md            v7 人工复核（对→错判分假阴性、错→对逐题核对）
...
1. 决定下一步方向：扩大样本到 200–300 题收窄置信区间，或先加大效应量（更宽窗口/更多重采样步数）再验证——100 题已给出方向正确但未达统计显著的结果。
2. 视需要为判分脚本 `normalize_answer` 补充 `\%`/`%` 尾部剥离（v7 中发现的假阴性），需与用户确认是否修改判分逻辑。
3. 视样本规模决定是否补 `alpha`（2 vs 4）与 `gain_threshold` 对照。

### 退化复跑判定（2026-07-20）

对 v5 两道乱码退化题的探针实验（各换 5 个种子重新初始生成）证实退化是**种子依赖的随机事件**，不是题目内在属性。据此实现复跑判定而非重复惩罚——重复惩罚会改变所有题的 proposal 分布、破坏 MH 接受公式中的 proposal 抵消，而复跑只重掷链的起点（不参与 MH 核），数学干净：

- `generation/degeneration.py`：退化判定（顶满预算且无 `\boxed{}`）+ 换种子重生成（默认最多 3 次，`--degeneration-retries` 可调）。
- 结果记录新增 `degeneration_retries`、`initial_degenerate` 字段；首发即正常的题随机流与旧版一致，历史 paired 结果可比。
- 离线测试 33/33 通过。
